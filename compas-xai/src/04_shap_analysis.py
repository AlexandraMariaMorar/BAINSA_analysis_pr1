...
def load_test_data(seed):
    ...
    if "row_id" not in split_data.columns:
        raise ValueError(
            "Split file must contain a row_id column."
        )

    if "split" not in split_data.columns:
        raise ValueError(
            "Split file must contain a split column."
        )
    ...
    if "row_id" not in data.columns:
        raise ValueError(
            "Clean dataset must contain row_id."
        )
...
def compute_global_importance(shap_values, X_test, seed):
...
def run_shap_robustness():
    all_importance = []

    for seed in [7, 21, 42]:
        print(f"\nComputing TreeSHAP for seed {seed}")
        ...
        if shap_values.values.shape != expected_shape:
            raise ValueError(
                f"Unexpected SHAP shape for seed {seed}: "
    ...
    output_path = (
        RESULTS_DIR
        / "shap_importance_across_seeds.csv"
    )
    ...
    print("\nSHAP importance across seeds:")
    print(
        combined_importance.sort_values(
            ["seed", "mean_abs_shap"],
...
def summarize_shap_stability(combined_importance):
    summary = (
        combined_importance
    ...
    print(
        f"\nSHAP stability summary saved to: "
        f"{output_path}"
    )
